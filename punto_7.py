...
# ========================================================================
    ...
    print(f"\n{'DESTILADO':-^80}")
    print(f"Flujo total de destilado: {U1:.4f} kmol/h")
    print(f"\nComposición del destilado (fracción molar):")
    print(f"  x_Benceno           = {x_stages[0][0]:.8f}")
    print(f"  x_Etilbenceno       = {x_stages[0][1]:.8f}")
    print(f"  x_1,4-Dietilbenceno = {x_stages[0][2]:.8f}")
    print(f"                        {'─' * 20}")
    print(f"  Σx                  = {np.sum(x_stages[0]):.8f}")
    
    print(f"\nFlujos molares en destilado (kmol/h):")
    print(f"  Benceno:              {D_benzene_real:.6f} kmol/h")
    print(f"  Etilbenceno:          {D_ethylbenzene_real:.6f} kmol/h")
    print(f"  1,4-Dietilbenceno:    {D_diethylbenzene_real:.6f} kmol/h")
    
    print(f"\n{'FONDOS':-^80}")
    print(f"Flujo total de fondos: {L_stages[-1]:.4f} kmol/h")
    print(f"\nComposición de fondos (fracción molar):")
    print(f"  x_Benceno           = {x_stages[-1][0]:.8f}")
    print(f"  x_Etilbenceno       = {x_stages[-1][1]:.8f}")
    print(f"  x_1,4-Dietilbenceno = {x_stages[-1][2]:.8f}")
    print(f"                        {'─' * 20}")
    print(f"  Σx                  = {np.sum(x_stages[-1]):.8f}")
    
    print(f"\nFlujos molares en fondos (kmol/h):")
    print(f"  Benceno:              {B_benzene_real:.6f} kmol/h")
    print(f"  Etilbenceno:          {B_ethylbenzene_real:.6f} kmol/h")
    print(f"  1,4-Dietilbenceno:    {B_diethylbenzene_real:.6f} kmol/h")
    
    print(f"\n{'RECUPERACIONES (%)':-^80}")
    print(f"Recuperación de Benceno en destilado:       {recovery_benzene_real:.4f}%")
    print(f"Recuperación de Etilbenceno en destilado:   {recovery_ethylbenzene_real:.4f}%")
    print(f"Recuperación de 1,4-Dietilbenceno:          {recovery_diethylbenzene_real:.4f}%")
    ...
    print(f"  Objetivo:    {objetivo_rec_benzene:>8.2f}%")
    print(f"  Obtenido:    {recovery_benzene_real:>8.4f}%")
    print(f"  Diferencia:  {recovery_benzene_real - objetivo_rec_benzene:>8.4f}%")
    cumple_benzene = "✓ CUMPLE" if recovery_benzene_real >= objetivo_rec_benzene - 0.1 else "✗ NO CUMPLE"
    print(f"  Estado:      {cumple_benzene}")
    
    print(f"\nRecuperación de Etilbenceno:")
    print(f"  Objetivo:    {objetivo_rec_ethylbenzene:>8.2f}%")
    print(f"  Obtenido:    {recovery_ethylbenzene_real:>8.4f}%")
    print(f"  Diferencia:  {recovery_ethylbenzene_real - objetivo_rec_ethylbenzene:>8.4f}%")
    cumple_ethylbenzene = "✓ CUMPLE" if abs(recovery_ethylbenzene_real - objetivo_rec_ethylbenzene) <= 0.5 else "✗ NO CUMPLE"
    print(f"  Estado:      {cumple_ethylbenzene}")
    
    print(f"\n{'BALANCE DE MATERIA':-^80}")
    print(f"\n{'Componente':<22} {'Entrada':<15} {'Destilado':<15} {'Fondos':<15} {'Error':<15}")
    print("-" * 82)
    ...
    for i, comp_name in enumerate(["Benceno", "Etilbenceno", "1,4-Dietilbenceno"]):
        ...
        print(f"{comp_name:<22} {F_comp:>14.6f}  {D_comp:>14.6f}  {B_comp:>14.6f}  {error:>14.8f}")
    ...
    print("-" * 82)
    print(f"{'TOTAL':<22} {F_total_check:>14.6f}  {D_total_check:>14.6f}  {B_total_check:>14.6f}  {error_total:>14.8f}")
    
    error_pct_total = (error_total / F_total_check) * 100
    print(f"\nError en balance de materia total: {error_pct_total:.8f}%")
    
    # Resumen compacto de composiciones
    print(f"\n{'RESUMEN DE COMPOSICIONES (FRACCIÓN MOLAR)':-^80}")
    print(f"\n{'Componente':<22} {'Destilado':<20} {'Fondos':<20}")
    print("-" * 62)
    print(f"{'Benceno':<22} {x_stages[0][0]:>19.8f} {x_stages[-1][0]:>19.8f}")
    print(f"{'Etilbenceno':<22} {x_stages[0][1]:>19.8f} {x_stages[-1][1]:>19.8f}")
    print(f"{'1,4-Dietilbenceno':<22} {x_stages[0][2]:>19.8f} {x_stages[-1][2]:>19.8f}")
